...
public class AntiAlignmentILPCalculator extends AbstractILPCalculator {
	...
	public static int getMaxLengthLog(int maxTraceLength, double maxFactor) {
		return (int) (maxTraceLength * 2 * maxFactor + 0.5);
	}
	...
	protected void solveSequential(int maxLength, Marking initialMarking, Marking finalMarking,
		...
		do {
			...
			if (VERBOSE) {
				assert checkFiringSequence(firingSequence, initialMarking, marking);
				assert checkHammingDistancesOnPrefix(hammingDistances, antiAlignment.toArray());
				assert startTracesAt == antiAlignment.size();
			}
			matrix = setupLpForHybrid(maxLengthX, maxLengthY, true, marking, finalMarking, traceToIgnore,
					startTracesAt);
			...
			if (nextResult == null) {
			...
			} else if (backTrackDepth < maxBackTrackDepth && nextResult.getLengthX()
					+ nextResult.getLengthY() < intermediate.getLengthY() / backtrackThreshold) {
				backTrackDepth++;
				...
				if (antiAlignment.isEmpty() && NAMES) {
				...
				Transition last;
				int evt = antiAlignment.size() - 1;//maxLength - maxLengthX - maxLengthY - 1;
				short label = antiAlignment.get(evt);
				antiAlignment.removeAt(evt);
				...
				if (VERBOSE) {
					assert checkFiringSequence(firingSequence, initialMarking, intermediate.getMarking());
				}
				...
				if (VERBOSE) {
					assert checkFiringSequence(firingSequence, initialMarking, nextResult.getMarking());
					assert checkHammingDistancesOnPrefix(hammingDistances, antiAlignment.toArray());
				}
		...
		if (VERBOSE) {
			...
			assert checkHammingDistances(hammingDistances, antiAlignment.toArray());
	...
	private boolean checkHammingDistancesOnPrefix(int[] hammingDistances, short[] antiAlignment) {
		...
		for (int t = 0; t < log.length; t++) {
			hdComputed[t] = getHammingDistanceToTrace(log[t], antiAlignment)
					- (log[t].length > antiAlignment.length ? log[t].length - antiAlignment.length : 0);
		}
		assert Arrays.equals(hdComputed, hammingDistances);
		return Arrays.equals(hdComputed, hammingDistances);
	}

	private boolean checkHammingDistances(int[] hammingDistances, short[] antiAlignment) {
		int[] hdComputed = new int[log.length];
		for (int t = 0; t < log.length; t++) {
			hdComputed[t] = getHammingDistanceToTrace(log[t], antiAlignment);
		}
	...
	private boolean checkFiringSequence(Vector<Transition> firingSequence, Marking initialMarking,
	...
	protected void solveByDrillingDown(final int splitInParts, int maxLength, Marking initialMarking,
		...
		if (VERBOSE) {
			System.out.println(
					"Solving maxlength " + maxLength + " from marking " + initialMarking + " to " + finalMarking);
		}
		if (maxLength > cutOffLength) {
			...
			if (intermediate != null && !initialMarking.equals(intermediate)) {
				...
				if (intermediate != null) {
					...
					if (result != LPMatrix.OPTIMAL) {
						...
						System.out.println(
								"Cannot reach the final marking " + finalMarking + " from " + initialMarking + "!");
					}
	...
	public boolean doAntiAlignmentTest(Marking initialMarking, Marking finalMarking) throws LPMatrixException {
		...
		while (cutOffLength < maxCutOffLength) {
			...
			for (int i = 0; i < exp; i++) {
				...
				antiAlignment = new TShortArrayList(getMaxLengthLog(maxLength, maxFactor));
				long start = System.currentTimeMillis();
				//				solveByDrillingDown(2, maxLength * maxFactor, initialMarking, finalMarking, firingSequence,
				//						antiAlignment, -1, 0);
				solveSequential(getMaxLengthLog(maxLength, maxFactor), initialMarking, finalMarking, firingSequence,
						antiAlignment, -1);
	...
	public AntiAlignments getAntiAlignments(Progress progress, Marking initialMarking, Marking finalMarking)
		...
		TShortList antiAlignment = new TShortArrayList(getMaxLengthLog(maxLength, maxFactor));
		...
		final AntiAlignments antiAlignments = new AntiAlignments(log.length, getMaxLengthLog(maxLength, maxFactor),
				maxFactor);
		...
		for (int t = 0; t < log.length && (progress == null || !progress.isCancelled()); t++) {
			...
			firingSequence = new Stack<Transition>();
			antiAlignment = new TShortArrayList(getMaxLengthRemovedTrace(log[t], maxFactor));
			//			solveByDrillingDown(maxFactor * log[t].length, initialMarking, finalMarking, firingSequence, antiAlignment,
			//					t, 0);
			start = System.currentTimeMillis();
			solveSequential(getMaxLengthRemovedTrace(log[t], maxFactor), initialMarking, finalMarking, firingSequence,
					antiAlignment, t);
	...
	protected LPMatrix<?> setupLpForFullSequence(int maxLength, boolean integerVariables, Marking initialMarking,
		...
		LPMatrix<?> lp = setupMatrix((maxLength + 1) * places + 2 * maxLength + log.length,
				transitions * maxLength + invisibleTransitions + 2);
	...
	protected LPMatrix<?> setupLpForHybrid(int maxLengthX, int maxLengthY, boolean integerVariables,
		...
		LPMatrix<?> lp = setupMatrix(
				places * (maxLengthX + 1) + (maxLengthX - 1) + maxLengthX + 1/*                     */
				//- y.1 - maxLength X_(lengthX-1).1 <=0 traceXHD  rem.traceHD   
						+ 1 //
						+ log.length,
				transitions * maxLengthX + transitions + 2);
		...
		// then X.1 <= 1
		for (row = (maxLengthX + 1) * places + maxLengthX - 1; row < (maxLengthX + 1) * places + 2 * maxLengthX
				- 1; row++) {
			rhs[row] = 1;
		}
		// Y.1 <= maxLengthY
	...
	protected HybridEquationResult determineSplitMarkingForHybrid(LPMatrix<?> matrix, int maxLengthX, int startTracesAt,
			boolean includeTrailingTaus, TShortList antiAlignment, Stack<Transition> firingSequence,
